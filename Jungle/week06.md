...
### 페이징과 세그멘테이션의 정의 및 각각의 장단점

세그멘테이션과 페이징은 모두 물리 메모리 관리 방법이다.

세그멘테이션
- 메모리를 의미 있는 단위인 세그먼트로 나누는 방법이다. 각 세그먼트는 시작 주소와 길이를 가진다.
- 각 세그먼트는 서로 다른 유형(코드, 데이터, 스택, 힙)의 데이터를 위해 사용된다.
- 세그멘테이션은 메모리를 더 유연하게 관리할 수 있게 해준다. 또한 프로그램의 논리적 구조를 반영할 수 있다.

장점
- 메모리를 논리적 단위로 나누어 프로그램의 구조를 반영한다.
- 세그먼트 별 보호와 공유가 용이하다.
...
페이징
- 메모리를 동일한 크기의 블록 단위, '페이지'로 나누는 방법이다. 
- 각 페이지는 가상 메모리 주소와 매핑되며, 페이지 테이블을 통해 물리적 메모리 주소로 변환된다.
- 페이징은 메모리 관리를 단순화하고, 메모리의 낭비를 줄인다. 또한 프로그램 간 메모리 충돌도 방지한다.
...
### DMA의 기본 개념을 설명하고, DMA가 시스템 성능에 미치는 이점을 두 가지 이상 제시

기본 개념
- DMA(Direct Memory Access)는 CPU 중재 없이 주변 장치가 메모리에 직접 접근하여 데이터를 읽고 쓸 수 있게 해주는 시스템의 한 기능이다. 
- DMA를 사용하여 데이터 전송과정에서 CPU가 필요하지 않기 때문에 CPU는 다른 작업을 수행하는 데 더 많은 시간을 할애할 수 있다.
- 이는 시스템 성능 향상으로 이어진다. 특히 I/O 작업이 많은 시스템에서 그 이점이 두드러진다.

시스템 서능에 미치는 이점
- CPU 부하가 감소하여 전체 시스템의 효율성이 증가한다.
- 데이터 전송 속도가 향상되어 전반적인 시스템 응답 시간이 단축된다.